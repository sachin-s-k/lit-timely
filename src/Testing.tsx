...
export const Testing = ({
  ...
  const handleTimeChange = (
    ...
    if (!updatedSlot.start || !updatedSlot.end) {
      updatedSlot.error = "Both times required";
    } else if (
      !validateTimeFormat(updatedSlot.start) ||
      !validateTimeFormat(updatedSlot.end)
    ) {
      updatedSlot.error = "Invalid time format";
    } else if (
      convertTo24Hour(updatedSlot.end) <= convertTo24Hour(updatedSlot.start)
    ) {
      updatedSlot.error = "End must be later";
    } else if (
      checkOverlap(
        updatedSlot,
        updatedAvailability[dayIndex].timeSlots.filter(
      ...
      updatedSlot.error = "Slots overlap";
    }
  ...
  return (
    <div
      className={`flex flex-col gap-4 ${
        isMobile ? "p-3" : "p-5"
      } border border-blue-100 rounded-md shadow-md`}
    >
      {[
        "Sunday",
        "Monday",
        ...
        <div
          key={day}
          className={`flex ${
            isMobile ? "flex-col gap-3" : "items-start gap-8"
          }`}
        >
          <div
            className={`flex items-start gap-2 ${
              isMobile ? "w-full justify-between" : "w-16"
            }`}
          >
            <div className="flex items-center gap-2">
              <input
                ...
                checked={availability[dayIndex]?.isAvailable}
                onChange={() => handleCheckboxChange(dayIndex)}
                className="w-4 h-4 accent-blue mt-1"
              />
              <span
                className={`font-bold ${isMobile ? "text-sm" : "text-md"} mt-1`}
              >
                {isMobile
                  ? day.slice(0, 3).toUpperCase()
                  : day.slice(0, 3).toUpperCase()}
              ...
              {availability[dayIndex].timeSlots.map(
                (slot: any, slotIndex: any) => (
                  <div
                    key={slotIndex}
                    className={`flex ${
                      isMobile ? "flex-col gap-2" : "flex-col gap-3"
                    } w-full`}
                  >
                    <div
                      className={`flex ${
                    ...
                    >
                      <input
                        ...
                        } h-10 border text-center rounded-md font-light text-sm text-gray-600 ${
                          slot.error ? "border-red-500" : "border-gray-300"
                        }`}
                      />
                      <input
                      ...
                      {slotIndex > 0 && (
                        <button
                          className="text-gray-600 hover:text-red-700 ml-1"
                          onClick={() => removeTimeSlot(dayIndex, slotIndex)}
                        >
                          <XIcon size={16} />
                        </button>
                ...
                (slot: any, slotIndex: any) => (
                  ...
                  >
                    ...
                    >
                      ...
                      {/* End Time - different for mobile/desktop */}
                      {isMobile ? (
                        <div className="w-full relative">
                          <input
                            ...
                            className={`w-full h-10 border text-center rounded-md font-light text-sm text-gray-600 pr-8 ${
                              slot.error ? "border-red-500" : "border-gray-300"
                            }`}
                          />
                          {slotIndex > 0 && (
                            <button
                              className="absolute right-2 top-1/2 transform -translate-y-1/2 flex items-center justify-center w-6 h-6 rounded-full bg-gray-200 hover:bg-gray-300 text-gray-600 hover:text-red-700"
                              onClick={() =>
      ...
      {update && (
        <div className={`${isMobile ? "w-full text-center" : "ml-40"}`}>
          <button
            onClick={() => createAvailability()}
            disabled={hasErrors() || !isAtLeastOneDayChecked()}
